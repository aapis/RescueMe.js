...
Todo.prototype = {
	...
	_setListeners: function(context){
		this.EVM.bind(document.querySelector('.new'), 'keyup', function(evt){
			...
			if(evt.which === 13 && evt.srcElement.value.length > 0){
				if(context.patterns.lists.test(evt.srcElement.value)){ //lists
					context.addList({
					...
					context.add({
						...
						srcElement: evt.srcElement,
						parent: context,
						isChild: !!context.patterns.add_to_list.test(evt.srcElement.value),
					});
				}
	...
	add: function(data, pass){
		...
		text = data.text.replace(this.patterns.add_to_list, '');
	...
	addList: function(data){
		...
		//if(this.queue.lists.length === 0)
		//	target.innerHTML = '';

		text = data.text.replace(this.patterns.lists, '');

		//setup the list element
		output.innerHTML = '<a href="#">'+ text + '<span class="close pull-right" href="#">&#x2713;</span></a>';
