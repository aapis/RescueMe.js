...
Todo.prototype = {
	...
	_setListeners: function(context){
		this.EVM.bind(document.querySelector('.new'), 'keyup', function(evt){
			if(evt.which === 13 && evt.srcElement.value.length > 0){
				context.add({
					...
					buffer: context,
					//type: (evt.srcElement.value.test(context.patterns.lists) ? 'lists' : 'task'), //will have to be rewritten for other types
				});
			}
		});

		/*
		 //probably not going to be used now that deleting items works
		...
		this.EVM.bind(document.querySelector('#check-all'), 'click', function(){
			...
			if(doDelete){
				...
				document.querySelector('#check-all').style.display = 'none';
			}
		});
	},
	...
	add: function(data, pass){
		...
		document.querySelector('#check-all').style.display = 'inline-block';
		...
		if(!this.patterns.lists.test(data.text)){ //test to see if it is a list
			...
			if(this.patterns.lists.test(data.text)){
				...
				output.classList.add('list'); //an example of how tags would work
			}
		}
