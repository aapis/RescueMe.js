...
Todo.prototype = {
	...
	_setInitalData: function(context){
		...
		this.EVM.bind(document.querySelector('li>span.check'), 'click', function(evt){
			...
			for(var i = 0, messages = data.parent.queue.tasks; i < messages.length; i++){
				if(data.text == messages[i]){
					context.add_to_list(messages.splice(i, 1), 'complete');
	...
	add: function(data, pass){
		...
		//delete an item control
		this.EVM.bind(outputParent.querySelector('span.delete'), 'click', function(evt){
			evt.preventDefault();
		...
		this.EVM.bind(outputParent.querySelector('span.check'), 'click', function(evt){
			evt.preventDefault();
	...
	add_to_list: function(item, list){
		if(item){
			this.add({
				text: 'al:'+ list +' '+ item,
				srcElement: null,
				parent: this,
				isChild: !!this.patterns.add_to_list.test(item),
				render: false,
			});
		}
